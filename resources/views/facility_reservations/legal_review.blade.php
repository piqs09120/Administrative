    ...
    <div class="flex flex-col flex-1 overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto bg-gray-50 p-6">
        <div class="flex items-center mb-6">
          ...
          <h1 class="text-3xl font-bold" style="color: var(--color-charcoal-ink);">Legal Review</h1>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div class="lg:col-span-2">
            <div class="card bg-white shadow-xl" style="background-color: var(--color-white); border-color: var(--color-snow-mist);">
              <div class="card-body">
                <h2 class="card-title text-xl mb-4" style="color: var(--color-charcoal-ink);">Reservation</h2>
                <div class="space-y-2 text-sm">
                  <div><strong>Facility:</strong> {{ $reservation->facility->name ?? 'N/A' }}</div>
                  <div><strong>Requester:</strong> {{ $reservation->reserver->name ?? 'N/A' }}</div>
                  <div><strong>Schedule:</strong> {{ \Carbon\Carbon::parse($reservation->start_time)->format('M d, Y H:i') }} – {{ \Carbon\Carbon::parse($reservation->end_time)->format('M d, Y H:i') }}</div>
                ...
                @php
                  ...
                  $ai = $documentTask ? ($documentTask->details['ai_classification'] ?? null) : null;
                @endphp

                @if($ai)
                  <div class="mt-6">
                    <h3 class="font-semibold mb-2" style="color: var(--color-charcoal-ink);">AI Analysis</h3>
                    <div class="space-y-1 text-sm">
                      <div><strong>Category:</strong> {{ ucfirst($ai['category'] ?? 'general') }}</div>
                      <div><strong>Risk:</strong> {{ $ai['legal_risk_score'] ?? 'Low' }}</div>
                      <div><strong>Summary:</strong> {{ $ai['summary'] ?? '' }}</div>
                      @if(!empty($ai['tags']))
                        <div><strong>Tags:</strong> {{ implode(', ', (array)$ai['tags']) }}</div>
                      @endif
                    </div>
                  </div>
                @endif
          ...
          <div class="lg:col-span-1">
            <div class="card bg-white shadow-xl" style="background-color: var(--color-white); border-color: var(--color-snow-mist);">
              <div class="card-body">
                <h3 class="card-title text-lg mb-2" style="color: var(--color-charcoal-ink);">Decision</h3>
                <form method="POST" action="{{ route('facility_reservations.legal_approve', $reservation->id) }}" class="mb-3">
                  @csrf
                  <textarea name="legal_comment" class="textarea textarea-bordered w-full mb-3" placeholder="Notes (optional)" style="color: var(--color-charcoal-ink);"></textarea>
                  <button type="submit" class="btn btn-primary w-full">
                    <i data-lucide="check" class="w-4 h-4 mr-2"></i>Approve Legal Review
                  </button>
                </form>
                <form method="POST" action="{{ route('facility_reservations.legal_flag', $reservation->id) }}">
                  @csrf
                  <textarea name="legal_comment" class="textarea textarea-bordered w-full mb-3" placeholder="Flag reason (optional)" style="color: var(--color-charcoal-ink);"></textarea>
                  <button type="submit" class="btn btn-error w-full" style="background-color: var(--color-danger-red); border-color: var(--color-danger-red); color: var(--color-white);">
                    <i data-lucide="flag" class="w-4 h-4 mr-2"></i>Flag & Deny
                  </button>
                </form>
              </div>
            </div>
